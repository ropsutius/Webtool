...
*/
  ...
  g.materialDefault =
    g.materialTopDefault &&
    g.materialBottomDefault &&
    g.materialCoverDefault &&
    g.fixedMaterialDefault;
  ...
  g.unrollCover =
    p.unrollCover !== undefined
      ? p.unrollCover
      : function (v, t) {
          return 0;
        };
  g.waffleDeep =
    p.waffleDeep !== undefined
      ? p.waffleDeep
      : function (u, v, t) {
          return 0;
        };
  ...
  g.endCircAngle =
    p.endCircAngle !== undefined
      ? p.endCircAngle
      : function (v, t) {
          return 1;
        };
  g.startCircAngle =
    p.startCircAngle !== undefined
      ? p.startCircAngle
      : function (v, t) {
          return 0;
        };
  g.scaleCircAngle =
    p.scaleCircAngle !== undefined
      ? p.scaleCircAngle
      : function (u, t) {
          return u;
        };
  g.topHeight =
    p.topHeight !== undefined
      ? p.topHeight
      : function (u, t) {
          return 1;
        };
  g.bottomHeight =
    p.bottomHeight !== undefined
      ? p.bottomHeight
      : function (u, t) {
          return 0;
        };
  g.scaleHeight =
    p.scaleHeight !== undefined
      ? p.scaleHeight
      : function (v, t) {
          return v;
        };
  ...
  if (p.materialCover !== undefined)
    g.materialCover = function (u, v, t) {
      return Math.floor(10 * p.materialCover(u, v, t));
    };
  if (p.materialBottom !== undefined)
    g.materialBottom = function (u, t) {
      return Math.floor(10 * p.materialBottom(u, t));
    };
  if (p.materialTop !== undefined)
    g.materialTop = function (u, t) {
...
function create() {
  ...
  if (g.indexed) {
    ...
    g.setAttribute(
      'position',
      new BufferAttribute(g.vertices, 3).setUsage(DynamicDrawUsage)
    );
    g.setAttribute(
      'normal',
      new BufferAttribute(g.normals, 3).setUsage(DynamicDrawUsage)
    );
    ...
    if (g.quadLine) {
      ...
      g.quadLine = new Line(
        g.lineGeometry,
        new LineBasicMaterial({ color: g.quadColor })
      );

      g.linePositions = new Float32Array(2 * hss * rss * 3);
      g.lineGeometry.setAttribute(
        'position',
        new BufferAttribute(g.linePositions, 3)
      );
    }
      ...
      for (var j = 0; j < rss; j++) {
        for (var i = 0; i < hss; i++) {
          ...
          if (j > 0) {
            if (i < hs) {
              vFace.push(fLeftPos);
              if (i === 0 && (!g.circOpen || (g.circOpen && j !== rs)))
                vFace.push(fLeftPos); // face double (equal number left and right)
            }
          ...
          if (j < rs) {
            ...
            if (i > 0) {
              vFace.push(fRightPos - 3);
              if (i === hs && (!g.circOpen || (g.circOpen && j !== 0)))
                vFace.push(fRightPos - 3); // face double (equal number left and right)
            }
          }
    ...
    if (g.withBottom || g.withTop) {
      ...
      if (g.withTop) {
        ...
        for (var j = 0; j < rss; j++) {
          ...
          if (j === 0 && !g.circOpen)
            vFace.push((hs * rs * fps + (g.withBottom ? rs : 0) + rs - 1) * 3);
          if (j === rs && !g.circOpen)
            vFace.push((hs * rs * fps + (g.withBottom ? rs : 0)) * 3);
          if (j > 0)
            vFace.push((hs * rs * fps + (g.withBottom ? rs : 0) + j - 1) * 3);
          if (j < rs)
            vFace.push((hs * rs * fps + (g.withBottom ? rs : 0) + j) * 3);
  ...
  if (!g.indexed) {
    ...
    g.setAttribute(
      'position',
      new BufferAttribute(g.positions, 3).setUsage(DynamicDrawUsage)
    );
    g.setAttribute(
      'normal',
      new BufferAttribute(g.normals, 3).setUsage(DynamicDrawUsage)
        ...
        for (var i = 0; i < hss; i++) {
          ...
          if (j < rs) {
            ...
            if (i > 0) {
              vFace.push(fRightPos - 9);
              if (i === hs && (!g.circOpen || (g.circOpen && j !== 0)))
                vFace.push(fRightPos - 9); // face double (equal number left and right)
...
function morphVertices(time) {
  ...
  function xyzCalculation() {
    scalingAngle =
      g.startCircAngle(ni, t) +
      (g.endCircAngle(ni, t) - g.startCircAngle(ni, t)) *
        g.scaleCircAngle(nj, t);

    // scalingHeight
    if (g.noCenterPoints) {
      scalingHeight =
        g.bottomHeight(nj, t) +
        (g.topHeight(nj, t) - g.bottomHeight(nj, t)) * g.scaleHeight(ni, t);
    } else {
      scalingHeight =
        g.bottomHeight(nj, t) +
        ((g.topHeight(nj, t) - g.bottomHeight(nj, t)) * scHeight[i]) / g.height;
    }
    ...
    if (g.style === 'map' || g.style === 'cover') {
      ...
      z =
        g.style === 'map' ? 0 : g.radius * g.rCircHeight(nj, ni, t) - g.radius;
    }
    ...
    if (g.style === 'complete') {
      ...
      if (
        ...
        if (g.noCenterPoints) {
          ...
          scalingHeight01 =
            g.bottomHeight(nj, t) +
            (g.topHeight(nj, t) - g.bottomHeight(nj, t)) *
              g.scaleHeight(ni01, t);
  ...
  if (!g.noCenterPoints) {
    ...
    for (var i = 0; i < g.centerPoints.length; i++) {
      if (i > 0) {
        ...
        scHeight[i] =
          scHeight[i - 1] + Math.sqrt(dX * dX + dY * dY + dZ * dZ) * g.height;
      }
    }
  }
  ...
  if (g.indexed) {
    ...
    for (var f = 0; f < faceCount; f++) {
      ...
      f1Vec.x =
        g.vertices[g.faceIndices[fIdx] * 3] -
        g.vertices[g.faceIndices[fIdx0] * 3];
      f1Vec.y =
        g.vertices[g.faceIndices[fIdx] * 3 + 1] -
        g.vertices[g.faceIndices[fIdx0] * 3 + 1];
      f1Vec.z =
        g.vertices[g.faceIndices[fIdx] * 3 + 2] -
        g.vertices[g.faceIndices[fIdx0] * 3 + 2];
  ...
  if (!g.indexed) {
    ...
    function setExplodedPositions() {
      ...
      x +=
        g.positions[posIdx] + g.positions[posIdx + 3] + g.positions[posIdx + 6];
      y +=
        g.positions[posIdx + 1] +
        g.positions[posIdx + 4] +
        g.positions[posIdx + 7];
      z +=
        g.positions[posIdx + 2] +
        g.positions[posIdx + 5] +
        g.positions[posIdx + 8];
    ...
    if (!g.explodeDefault) {
      if (g.explodemode === 'center') {
        ...
        if (g.waffled) {
          ...
          for (var j = 0; j < rs; j++) {
            ...
            for (var i = 0; i < hs; i++) {
              ...
              y =
                g.positions[g.vertexPositions[waffleVidx][0] + 1] *
                g.explode(t);
              z =
                g.positions[g.vertexPositions[waffleVidx][0] + 2] *
                g.explode(t);
        ...
        if (g.withTop) {
          for (
            var f = hs * rs * fps + (g.withBottom ? rs : 0);
            f < hs * rs * fps + (g.withBottom ? 2 * rs : rs);
            f++
          ) {
            setExplodedPositions();
          }
      ...
      if (g.explodemode === 'normal') {
        ...
        for (var f = 0; f < faceCount; f++) {
          for (var p = 0; p < 3; p++) {
            ...
            g.positions[posIdx + p * 3] =
              g.positions[posIdx + p * 3] +
              g.faceNormals[f * 3] * g.radius * g.explode(t);
            g.positions[posIdx + p * 3 + 1] =
              g.positions[posIdx + p * 3 + 1] +
              g.faceNormals[f * 3 + 1] * g.radius * g.explode(t);
            g.positions[posIdx + p * 3 + 2] =
              g.positions[posIdx + p * 3 + 2] +
              g.faceNormals[f * 3 + 2] * g.radius * g.explode(t);
          }
        }
      }
    }
...
function morphFaces(time) {
  if (!g.materialDefault) {
      ...
      if (!g.waffled) {
        ...
        for (var j = 0; j < rs; j++) {
          ...
          for (var i = 0; i < hs; i++) {
            ...
            g.groups[fIdx].materialIndex = g.materialCover(
              (j + 0.5) / rs,
              i / hs,
              t
            ); // by function
    ...
    if (g.withBottom || g.withTop) {
      ...
      if (
        g.withBottom &&
        (!g.materialBottomDefault || g.fixedMatBottom !== '')
      ) {
        fIdx = hs * rs * fps;
...
export { createMorphGeometry, create, morphVertices, morphFaces };
